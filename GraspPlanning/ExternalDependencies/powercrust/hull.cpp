...
#define snkey(x) pc->site_numm((x)->vert)
    ...
    if (!faces_gr_t) POWERCRUST_NEWLRC(fg, faces_gr_t);
    f = faces_gr_t;
...
void PowerCrustHull::visit_fg_i(   void (PowerCrustHull::*v_fg)(Tree *, int, int),
    ...
    if (t->fgs->mark!=vn) {
        t->fgs->mark = vn;
        if (t->key!=infinity && !mo[pc->site_numm(t->key)]) boundaryc = 0; 
...
void PowerCrustHull::p_fg(Tree* t, int depth, int bad) {
    ...
    if (mults[0]==0) {
        mults[pdim] = 1;
        for (i=pdim-1; i>=0; i--) mults[i] = pc->mult_up*mults[i+1];
    }
...
/* 
    ...
    while (tms) {
        ...
        t->visit = vnum;
        if ((v=(*this.*visit)(t,0))!=NULL) {return v;}
...
int PowerCrustHull::truet(simplex *s, int i, void *dum) {return 1;}
int PowerCrustHull::hullt(simplex *s, int i, void *dummy) {return i>-1;}
void *PowerCrustHull::facet_test(simplex *s, void *dummy) {return (!s->peak.vert) ? s : NULL;}
...
neighbor *PowerCrustHull::op_simp(simplex *a, simplex *b) {lookup(a,b,simp,simplex)}
/* the neighbor entry of a containing b */

neighbor *PowerCrustHull::op_vert(simplex *a, site b) {lookup(a,b,vert,site)}
/* the neighbor entry of a containing b */
...
simplex *PowerCrustHull::search(simplex *root) {
	...
	*(st+tms++) = root->peak.simp;//push(root->peak.simp);
    ...
    if (!sees(pc->p,root))
	{
		...
		logHull.log("#1#");	
#endif
		for (i=0,sn=root->neigh;i<cdim;i++,sn++) *(st+tms++) = sn->simp;//push(sn->simp);
	}
    while (tms) {
#ifdef HULL_DEBUG_TO_FILE
		logHull.log(tms);	
		...
		for (i=0, sn=s->neigh; i<cdim; i++,sn++) *(st+tms++) = sn->simp;//push(sn->simp);
    }
#ifdef HULL_DEBUG_TO_FILE
...
int PowerCrustHull::out_of_flat(simplex *root, point p) {
    ...
    get_basis_sede(root);
    if ((vd || pc->power_diagram)&& root->neigh[0].vert == infinity) return 1;
...
int PowerCrustHull::check_perps(simplex *s) {
    ...
    for (i=1;i<cdim;i++) {
        y = s->neigh[i].vert;
        if ( (vd || pc->power_diagram)&& y==infinity) memcpy(b, infinity_basis, basis_s_size);
...
void PowerCrustHull::get_normal(simplex *s) {get_normal_sede(s); return;}
...
#endif
        get_normal_sede(s);
        for (i=0;i<cdim;i++) POWERCRUST_NULLIFY(basis_s,s->neigh[i].basis);
    }
    tt = s->neigh[0].vert;
#ifdef HULL_DEBUG_TO_FILE
	logHull.log(tt[0]);	
...
void *PowerCrustHull::zero_marks(simplex * s, void *dum) { s->mark = 0; return NULL; }

void *PowerCrustHull::one_marks(simplex * s, void *dum) {s->mark = 1; return NULL;}

void *PowerCrustHull::show_marks(simplex * s, void *dum) {printf("%d",s->mark); return NULL;}
...
double PowerCrustHull::find_alpha(simplex *root) {
    ...
    for (i=0;i<pdim;i++) ah += (float)((pc->maxs[i]-pc->mins[i])*(pc->maxs[i]-pc->mins[i]));
...
void PowerCrustHull::vols(fg *f, Tree *t, basis_s* n, int depth) {
    ...
    if (!s) {POWERCRUST_NEWL(simplex,s); sn = s->neigh;}
    cdim = depth;
    s->normal = n;
...
void PowerCrustHull::set_ch_root(simplex *s) {ch_root = s; return;}
...
//		Huge = DBL_MAX*DBL_MAX;
    ...
    assert(get_s!=NULL); assert(site_numm!=NULL);
