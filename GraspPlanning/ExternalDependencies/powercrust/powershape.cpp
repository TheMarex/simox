...
int compare(const void *n1,const void *n2){
    ...
    if(index1 >= (int)(pshape.verts.size()) || index2 >= (int)(pshape.verts.size())){
        cout<<"Error!..\n"<<index1<<"\t"<<index2<<"\n";
        return 0;
    }
        ...
        for(size_t j=0;j<verts[currnode].neighbors.size();j++) {
            ...
            if((distance(currnode,currneighbor)-
               ...
               redThreshold){
                ...
                for(size_t k=0;k<verts[currneighbor].neighbors.size();k++)
...
int parseCommand(int argc,char **argv) {
    ...
    for(int i=1;i<argc;) {
        ...
        case 'r':rt=atof(argv[i+1]);
            i=i+2;
            break;
        case 'p':plotGraph=(bool)(atoi(argv[i+1])>0);
            i=i+2;
            break;
        case 'u':outputUnlabeled = (bool)(atoi(argv[i+1])>0);
            i+=2;
            break;
