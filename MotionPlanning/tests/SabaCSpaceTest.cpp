...
BOOST_AUTO_TEST_CASE(testCollisionModel)
{
	...
	d = cspace->calcDist(p1,p2);
	BOOST_CHECK_EQUAL(d,(float)M_PI*0.5f);
	...
	p2(0) = 0.75f * (float)M_PI;
	d = cspace->interpolate(p1,p2,0,0);
	BOOST_CHECK_EQUAL(d,-0.75f * (float)M_PI);
	d = cspace->interpolate(p1,p2,0,1);
	BOOST_CHECK_EQUAL(d,0.75f * (float)M_PI);
	d = cspace->interpolate(p1,p2,0,0.1f); // dist is 0.5PI -> result (-0.75 - 0.1*0.5) * PI
	BOOST_CHECK_EQUAL(d,-0.75f * (float)M_PI - 0.1f*0.5f*(float)M_PI);
	d = cspace->interpolate(p1,p2,0,0.4f);
	BOOST_CHECK_EQUAL(d,-0.75f * (float)M_PI - 0.4f*0.5f*(float)M_PI);
