...
#define BZ_GET_SMALL(cccc)                            \
	...
	if (s->tPos >= (UInt32)100000 * (UInt32)s->blockSize100k) return BZ_True; \
...
#define BZ_GET_FAST_C(cccc)                   \
	/* c_tPos is unsigned, hence test < 0 is pointless. */ \
	if (c_tPos >= (UInt32)100000 * (UInt32)ro_blockSize100k) return BZ_True; \
	c_tPos = c_tt[c_tPos];                    \
...
/*---------------------------------------------------*/
void CompressionBZip2::BZ2_bsInitWrite ( EState* s )
{
   s->bsLive = 0;
   s->bsBuff = 0;
}
...
/*---------------------------------------------------*/
void CompressionBZip2::bsFinishWrite ( EState* s )
{
   while (s->bsLive > 0) {
...
/*---------------------------------------------------*/
void CompressionBZip2::bsW ( EState* s, Int32 n, UInt32 v )
{
   bsNEEDW ( n );
...
/*---------------------------------------------------*/
void CompressionBZip2::bsPutUInt32 ( EState* s, UInt32 u )
{
   bsW ( s, 8, (u >> 24) & 0xffL );
   bsW ( s, 8, (u >> 16) & 0xffL );
   bsW ( s, 8, (u >>  8) & 0xffL );
...
/*---------------------------------------------------*/
void CompressionBZip2::bsPutUChar ( EState* s, UChar c )
{
   bsW( s, 8, (UInt32)c );
}

/*---------------------------------------------------*/
void CompressionBZip2::makeMaps_e ( EState* s )
{
   Int32 i;
   s->nInUse = 0;
   for (i = 0; i < 256; i++)
...
/*---------------------------------------------------*/
void CompressionBZip2::generateMTFValues ( EState* s )
{
   UChar   yy[256];
   Int32   i, j;
   Int32   zPend;
...
void CompressionBZip2::sendMTFValues ( EState* s )
{
...
#           undef BZ_ITUR
   ...
   { 
      ...
      for (i = 0; i < 16; i++) {
          inUse16[i] = BZ_False;
          for (j = 0; j < 16; j++)
             if (s->inUse[i * 16 + j]) inUse16[i] = BZ_True;
      }
...
*/
void CompressionBZip2::BZ2_blockSort ( CompressionBZip2::EState* s )
{
   UInt32* ptr    = s->ptr; 
   UChar*  block  = s->block;
   UInt32* ftab   = s->ftab;
...
void CompressionBZip2::mainSort ( UInt32* ptr, 
                UChar*  block,
                UInt16* quadrant, 
                UInt32* ftab,
                Int32   nblock,
...
CompressionBZip2::Int32 CompressionBZip2::incs[14] = { 1, 4, 13, 40, 121, 364, 1093, 3280,
                   9841, 29524, 88573, 265720,
                   797161, 2391484 };

void CompressionBZip2::mainSimpleSort ( UInt32* ptr,
                      UChar*  block,
                      UInt16* quadrant,
                      Int32   nblock,
                      Int32   lo, 
...
CompressionBZip2::UChar CompressionBZip2::mmed3 ( UChar a, UChar b, UChar c )
{
   UChar t;
   if (a > b) { t = a; a = b; b = t; };
   if (b > c) { 
...
void CompressionBZip2::mainQSort3 ( UInt32* ptr,
                  UChar*  block,
                  UInt16* quadrant,
                  Int32   nblock,
                  Int32   loSt, 
...
void CompressionBZip2::fallbackSort ( UInt32* fmap, 
                    UInt32* eclass, 
                    UInt32* bhtab,
                    Int32   nblock,
                    Int32   verb )
...
void CompressionBZip2::fallbackSimpleSort ( UInt32* fmap, 
	UInt32* eclass, 
	Int32   lo, 
	Int32   hi )
{
...
void CompressionBZip2::fallbackQSort3 ( UInt32* fmap, 
                      UInt32* eclass,
                      Int32   loSt, 
                      Int32   hiSt )
{
...
/*---------------------------------------------------*/
void CompressionBZip2::makeMaps_d ( DState* s )
{
   Int32 i;
   s->nInUse = 0;
   for (i = 0; i < 256; i++)
...
/*---------------------------------------------------*/
CompressionBZip2::Int32 CompressionBZip2::BZ2_decompress ( DState* s )
{
   UChar      uc;
   Int32      retVal;
   Int32      minLen, maxLen;
   ...
   switch (s->state) {
      ...
      for (i = 0; i < 16; i++) {
         ...
         if (uc == 1) 
            s->inUse16[i] = BZ_True; else 
            s->inUse16[i] = BZ_False;
      }

      for (i = 0; i < 256; i++) s->inUse[i] = BZ_False;

      for (i = 0; i < 16; i++)
         if (s->inUse16[i])
            for (j = 0; j < 16; j++) {
               GET_BIT(BZ_X_MAPPING_2, uc);
               if (uc == 1) s->inUse[i * 16 + j] = BZ_True;
            }
      makeMaps_d ( s );
...
CompressionBZip2::BZFILE* CompressionBZip2::BZ2_bzWriteOpen
                    ( int*  bzerror,      
                      //FILE* f, 
					  std::ostream *f,
                      int   blockSize100k, 
...
/*---------------------------------------------------*/
int CompressionBZip2::BZ2_bzCompressInit 
	( bz_stream* strm, 
	int        blockSize100k,
	int        verbosity,
	int        workFactor )
...
/*---------------------------------------------------*/
void CompressionBZip2::init_RL ( CompressionBZip2::EState* s )
{
	s->state_in_ch  = 256;
	s->state_in_len = 0;
}
...
/*---------------------------------------------------*/
void CompressionBZip2::prepare_new_block ( CompressionBZip2::EState* s )
{
	Int32 i;
	s->nblock = 0;
	s->numZ = 0;
...
CompressionBZip2::Bool CompressionBZip2::isempty_RL ( CompressionBZip2::EState* s )
{
	if (s->state_in_ch < 256 && s->state_in_len > 0)
		return BZ_False; else
		return BZ_True;
}

/*---------------------------------------------------*/
void CompressionBZip2::BZ2_bzWrite
             ( int*    bzerror, 
               BZFILE* b, 
               void*   buf, 
               int     len )
...
/*---------------------------------------------------*/
void CompressionBZip2::BZ2_bzWriteClose
                  ( int*          bzerror, 
                    BZFILE*       b, 
                    int           abandon,
...
void CompressionBZip2::BZ2_bzWriteClose64
                  ( int*          bzerror, 
                    BZFILE*       b, 
                    int           abandon,
...
/*---------------------------------------------------*/
int CompressionBZip2::BZ2_bzCompressEnd(CompressionBZip2::bz_stream *strm )
{
	EState* s;
...
/*---------------------------------------------------*/
int CompressionBZip2::BZ2_bzDecompressEnd( CompressionBZip2::bz_stream *strm )
{
	DState* s;
	if (strm == NULL) return BZ_PARAM_ERROR;
	s = (DState*)strm->state;
...
void CompressionBZip2::flush_RL ( CompressionBZip2::EState* s )
{
...
/*---------------------------------------------------*/
void CompressionBZip2::add_pair_to_block ( CompressionBZip2::EState* s )
{
	Int32 i;
...
/*---------------------------------------------------*/
CompressionBZip2::BZFILE* CompressionBZip2::BZ2_bzReadOpen 
                   ( int*  bzerror, 
					 std::istream *f,
                     int   verbosity,
					 int   smallValue,
...
/*---------------------------------------------------*/
void CompressionBZip2::BZ2_bzReadClose( int *bzerror, BZFILE *b )
{
   bzFile* bzf = (bzFile*)b;
...
/*---------------------------------------------------*/
int CompressionBZip2::BZ2_bzRead
           ( int*    bzerror, 
             BZFILE* b, 
             void*   buf, 
             int     len )
{
   ...
   while (BZ_True) {
      ...
      if (ret == BZ_OK && myfeof(bzf->handleIn) && 
...
CompressionBZip2::Bool CompressionBZip2::unRLE_obuf_to_output_FAST ( CompressionBZip2::DState* s )
{
   ...
   if (s->blockRandomised) {
      ...
      while (BZ_True) {
         ...
         while (BZ_True) {
            if (s->strm->avail_out == 0) return BZ_False;
         ...
         if (s->nblock_used == s->save_nblock+1) return BZ_False;
...
bool CompressionBZip2::write(void* buf, int len)
{
    if (!buf || len<=0)
        return false;
    if (mode!=eCompress)
...
bool CompressionBZip2::read(void* buf, int maxLen, int &storeLengthRead)
{
    if (!buf || maxLen<=0)
        return false;
    if (mode!=eUncompress)
...
CompressionBZip2::Int32 CompressionBZip2::BZ2_indexIntoF ( CompressionBZip2::Int32 indx, Int32 *cftab )
{
	Int32 nb, na, mid;
	nb = 0;
	na = 256;
...
/*---------------------------------------------------*/
void CompressionBZip2::BZ2_hbMakeCodeLengths ( UChar *len, 
                             Int32 *freq,
                             Int32 alphaSize,
                             Int32 maxLen )
{
...
/*---------------------------------------------------*/
void CompressionBZip2::BZ2_hbAssignCodes ( Int32 *code,
                         UChar *length,
                         Int32 minLen,
                         Int32 maxLen,
                         Int32 alphaSize )
...
/*---------------------------------------------------*/
void CompressionBZip2::BZ2_hbCreateDecodeTables ( Int32 *limit,
                                Int32 *base,
                                Int32 *perm,
                                UChar *length,
                                Int32 minLen,
...
int CompressionBZip2::read(char *res, int num, std::istream *file)
{
	if (!file)
		return 0;
...
bool CompressionBZip2::write(std::ostream *file, const char *value, int num)
{
	if (!file)
		return false;
